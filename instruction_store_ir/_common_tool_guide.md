...
#### 可用知识库目录（v4.8，自建研报库为主力源）

| KB ID | 名称 | 内容特色 | fetch | 最佳使用场景 |
|-------|------|---------|-------|-------------|
| `001a89fa4b807b92` | ★自建研报库（主力源） | 投行/券商研报（GS/MS/JPM/BofA/Citi/UBS/Bernstein 等），按周分文件夹 | ✅ **全文** | 估值方法论/目标价/BOM 成本/行业深度，**所有搜索第一优先** |
...
|------|---------|-------------------------------|
| **行业深度/竞争格局/TAM** | 自建研报库 + 行研智库 | 投行行研报告比 web 深度高 10 倍，比 NeoData doc 覆盖广 |
| **公司基本面/估值/目标价** | 自建研报库 | 投行研报含目标价方法论、盈利预测和估值逻辑（全文可取） |
| **机构观点/市场共识** | 自建研报库 + 机构调研纪要 | 电话会纪要、专家交流是公开源搜不到的 |
| **外资视角/跨境对比** | 自建研报库 | GS/MS/JPM/UBS 等外资券商研报独立视角 |
| **产业链上下游调研** | 自建研报库 + 行研智库 | 投行产业专家的一线调研纪要 |
| **政策/监管解读** | 行研智库 | 机构级政策解读报告 |

#### 搜索策略建议

1. **Step1（数据收集）**：必搜自建研报库 + 机构调研纪要，用 "{公司名}" 和 "{行业名}" 各搜一轮，建立机构信息基线
2. **Step2（行业分析）**：重点搜自建研报库 + 行研智库，用行业关键词 + 竞争格局 + TAM
3. **Step3（商业模式）**：搜自建研报库，关注客户/订单/收入结构
4. **Step4（财务分析）**：搜自建研报库，关注盈利预测和估值逻辑
5. **Step6b（估值）**：搜自建研报库，看外资估值方法和可比公司
...
示例：
```
[^15]: IMA知识库 — 自建研报库 — "Goldman Sachs: 人形机器人产业链深度，BOM成本拆解与量产指引" (2026-07-21)
...
- **全文提取**：search 命中后取最相关 1-3 篇 media_id → fetch_media_content 读全文（自建研报库/行研智库/精选报告均可 fetch）
- **搜索结果可能很长**：提取关键数据点和结论即可，不要全文照搬
- **时间过滤**：优先最近 30 天内的投行研报（超 1 个月参考价值显著下降），大行优先
