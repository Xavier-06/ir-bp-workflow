...
| 调查问题 | 首选工具 | 说明 |
|---------|---------|------|
| 企业风险全面扫描（35项） | `mcp__qcc-risk__get_company_risk_scan` | **红队核心入口**：一次扫描定位需下钻维度 |
| 司法文书明细 | `mcp__qcc-risk__get_judicial_documents` | 诉讼案由、案号、原被告、审理法院 |
| 失信被执行人 | `mcp__qcc-risk__get_dishonest_info` | 涉案金额、执行法院、立案/发布日期 |
| 行政处罚 | `mcp__qcc-risk__get_administrative_penalty` | 处罚结果、日期、金额、机关 |
| 经营异常 | `mcp__qcc-risk__get_business_exception` | 列入日期、移出原因、决定机关 |
| 股权冻结 | `mcp__qcc-risk__get_equity_freeze` | 冻结股权数额、冻结期限、执行法院 |
| 高消费限制 | `mcp__qcc-risk__get_high_consumption_restriction` | 限制对象、立案日期 |
| 历史股东变更 | `mcp__qcc-history__get_historical_shareholders` | 已退出股东、历史股权结构 |
| 历史失信/司法 | `mcp__qcc-history__get_historical_dishonest` / `get_historical_judicial_docs` | 已移出失信名单、已结案司法文书 |
| 客户/供应商工商存续 | `mcp__qcc-company__get_company_registration_info` | 验证关键合作方是否存续正常 |
| 客户/供应商股东（关联交易） | `mcp__qcc-company__get_shareholder_info` | 一层股东构成、识别隐性关联 |
| 负面新闻/舆情/举报 | `web_search` + `web_fetch` | 搜媒体报道、行业投诉、监管通报 |
| 正向叙事中的事实链验证 | `web_search` + QCC 工具交叉验证 | 交叉验证前置维度引用的关键事实 |
...
⚠️ 红队分析必须**主动用企查查风险工具做全面扫描**——不能只靠 `web_search` 搜新闻。企查查能发现尚未被报道的诉讼、行政处罚和股权冻结。
