...
**强制要求**：对每个核心技术产品，必须从 IMA 研报全文 + BP 中提取并列出**工程 spec 硬参数表**，每项参数带数值区间和单位，标注来源（研报/BP自述/第三方测试）：
- **材料/结构参数**：如孔容（cm³/g）、比表面积 BET（m²/g）、电导率/离子电导率（S/cm）、压实密度（g/cm³）、理论/实际比容量（mAh/g）、首次库伦效率 ICE（%）
- **门槛参数对照**：每个 spec 必须给出"行业实用区间"作为锚（如孔容实用区间 0.5–2.0 cm³/g、载硅骨架裸碳 BET 上限 ~2000 m²/g、电导率门槛 >2 S/cm），让读者知道标的参数处于什么水平
- **失效模式量化**：如硅嵌锂体积膨胀 ~300%、硫化物全固态循环寿命仅为液态 1/5–1/3

**成本颗粒度硬要求**（技术经济性，不能只说"成本高/低"）：
- **四档电芯/产品价**：如聚合物/氧化物/硫化物/卤化物电芯价分别多少 元/Wh，对比液态基准价
- **材料吨价**：关键材料单吨成本（如高纯 Li₂S >300 万元/吨、聚合物电解质 50–60 万元/吨、氧化物 30–40 万元/吨、液态电解液 1.7–3 万元/吨），给出差距倍数（如 10–100 倍）
- **降本时间线**：量产初期→规模化→成熟期的价格演进预测（带年份节点）
- **标的成本锚**：BP 自述的单位成本/售价（如煤基多孔炭原料 2000 元/吨、聚合物电解质售价比液态高 30%），标注"BP自述、待核验"

**前驱体/工艺路线经济学**（如适用，如多孔炭、负极材料等多前驱体路线）：
- 列出 ≥3 条前驱体/工艺路线（如生物质/树脂/煤基），每条给：优缺点、成本量级、标的采用/潜在采用标注
- 给出行业共识（如"树脂走高端、生物质走低端，两线并行"）+ 各路线后续降本/提性方向

**壁垒对标基准**：判断标的工艺是否通用技术时，必须给出对标基准（如"H₂O/CO₂ 物理活化是行业通用法，孔径精度 ±2nm 和 BET 上限不如 KOH 化学活化法/模板法"），不能只说"壁垒薄"。
