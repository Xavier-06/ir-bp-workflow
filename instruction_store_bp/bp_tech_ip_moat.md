...
**Step 3: IMA 技术横评搜索（与 Step 2 并行，不是兜底）**
- 行研智库 `7311568991699459`: `ima-mcp.search_knowledge` 搜 `{技术/行业名} 技术路线 对比 横评 壁垒` → 取最相关 1-3 篇结果 `fetch_media_content` 读全文
- 长安投研 `7297585010204027`: `ima-mcp.search_knowledge` 搜 `{公司/技术名} 技术 壁垒 护城河 专利`（加 TXT 过滤）→ 用 `introduction` 摘要
- 每库最多取 top 5 结果
