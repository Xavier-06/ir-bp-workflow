...
### Step 5: IMA 机构知识（ima-mcp — 自建研报库为主力源，全文可 fetch）
搜 web 搜索拿不到的机构级内容：
- `ima-mcp.search_knowledge`: knowledge_base_id='001a89fa4b807b92'（★自建研报库，GS/MS/JPM/BofA/Citi/UBS/Bernstein 等投行研报），query='{ENTITY} OR [行业] 研报 目标价 估值 竞争格局' → 卖方研报全文
...
- 有搜索结果时，对 top 1-3 最相关 media_id 调 `ima-mcp.fetch_media_content` 读全文（自建研报库/行研智库均可 fetch）
...
| 投行研报全文 | ima-mcp `search_knowledge` → `fetch_media_content` | 自建研报库为主力源，全文可 fetch |
