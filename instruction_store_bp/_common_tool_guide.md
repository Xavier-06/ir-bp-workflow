...
# Step 2: 全文提取 → 取最相关 5-8 篇结果的 media_id（多源交叉验证）
...
2. **多源交叉验证**: 关键数据（市场规模/估值/客户/专利）至少 2 个**独立机构**来源确认
   - **"独立"判定（2026-08-27 强化）**：同一研报被多家媒体/转引机构转载**不算**独立来源——判断标准是**原始测算机构**是否不同
   - 例：国信证券与 EVTank 分别测算固态电解质市场规模 = 2 个独立来源；国信证券数据被深企投产业研究院、华源证券引用 = 仍是 1 个来源（原始口径均为国信）
   - 只有 1 个独立来源时，数据可用但必须标注"单一机构口径（{机构名}）"，不得写成"行业共识"
...
## 表格与来源署名规范（2026-08-27 新增）

- **外部研报图表不得作为成品来源行裸奔**：维度输出中引用外部研报的表格/图表时，"资料来源：XX证券"或"深企投产业研究院整理"这类署名行属于溯源信息，必须转为 [^N] 脚注格式，不得在成品中以"资料来源："行形式独立出现
- **转引链条要写清**：若数据来自"研报 A 引用的报告 B"，脚注必须标注完整转引链（如"国信证券（2025-06），转引深企投产业研究院《XX》"），不得只写转引机构名让读者误以为是该机构原创测算
- **图表编号归属**：外部研报的图表编号（如"华源 260207 图表12"）只出现在脚注中；成品正文自身的图表从 1 开始独立连续编号，不得沿用外部编号
