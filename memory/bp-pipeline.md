...
### 2026-04-04 — 管线漏洞修复（8/9 通过压测）
- 搜索网关统一：所有脚本接入 `search_gateway.py`（SearXNG EN+CN+DDG 自动 fallback）
- LLM 重试：`extract_content.py` max_retries=3，指数退避 2s/4s/8s
- Phase 4 质量门禁：`_check_subagent_quality()` — 内容<500字符、占位标记≥3、缺少来源URL
- 域名黑名单：bp_presearch + extract_content 两处过滤（8 个低质量域名）
- 噪音词调宽：保留核心 6 个，移除 8 个过筛词
- 引擎健康检查：管线启动时先检测 3 引擎
- 端到端 timeout：`run_bp_pipeline.py` 加 timeout_sec 参数



- **2026-04-01**: 飞书消息 - 周欣发文件请求研究
- **2026-04-01**: 2026-04-01 16:17 — 静远达智 8 维度尽调报告
...
Phase 1-6 记忆系统升级全部完成

P0 实施完成：BP/IR 管线 Phase 0.5 + Phase 1 并行执行

Claude Code 源码分析完成 — 6 个可借鉴设计

记忆系统全面 Bug 修复（v4 修复完成）

最终端到端验证_20260403

### 记忆系统 Bug 修复报告（2026-04-03 22:30） (*2026-04-04*)

### 已修复（8 项）

| # | Bug | 修复方式 |
|---|-----|---------|
| 🔴 1 | `memory_bridge.py` sys.path 指向旧 `memory_system/`（v1 6KB）而非新 `memory_agent/`（v3 17KB） | 修正 sys.path 为 `ROOT / 'memory_agent'` |
| 🔴 2 | `memory_store.py` `_check_duplicate()` 错误解析 ChromaDB 返回 — `res["documents"][0][0]` 遍历字符串字符而非文档列表 | 改为 `zip(res["documents"][0], res["distances"][0])` |
| 🔴 3 | `memory_store.py` `search()` 查询不包含 `"distances"`（只有 documents+metadatas），但后续访问 `res["distances"][0]` → TypeError | query include 添加 `"distances"` |
| 🟡 4 | `memory_system/memory_store.py`（旧版 6KB）与 `memory_agent/memory_store.py`（新版 17KB）共存，版本混乱 | memory_bridge.py 已指向新版本（旧版保留兼容但不使用） |
| 🟡 5 | `memory-decay.sh` 使用 `/opt/homebrew/bin/python3`（3.14）但 chromadb 只装在 system python3（3.9.6）| 安装 chromadb 到 Python 3.14（`pip install --break-system-packages`）|
| 🟡 6 | memory/ 目录混入 timestamped transcript 文件（2026-04-03-0914.md, 2026-04-03-0919.md）| 移至 memory/transcripts/ 子目录 |
| 🟡 7 | `memory_dedup.py` 对 2026-04-03.md 未自动添加 frontmatter | 手动补写（后续 daily_log_create.py 会自动处理）|
| 🟡 8 | memory_dedup `_try_write_bridge` 的 `from memory.memory_bridge import add_memory` 依赖记忆 bridge | bridge 现在通过 memory_agent/venv subprocess 执行（subprocess 链路正常）|

### 无法修复 / 已知问题

| # | 问题 | 说明 |
|---|------|------|
| ⚪ 1 | 缺失 6 天日志（3/11,12,13, 21,22,26）| 周末/无活动日，daily-log launchd 正常但 agent 没触发写入 |
| ⚪ 2 | memory_dedup 写入 ChromaDB 时 category 映射不完全匹配 | `project` → `key_data`，但 `_try_write_bridge` 内部映射已覆盖大部分场景 |

### 修复后状态

- ChromaDB: user_preferences=10, past_errors=34, key_data=8, conversations=8
- 日志文件: 21 个 memory/*.md + 3 个 archives/ + 8 个 transcripts/
- all launchd 任务: OK（exit code 0）
- 向量搜索: ✅ 正常（含 age_label + freshness_warning）
- dedup 写入: ✅ 正常（文件 + 向量库双写）

### 管线升级（v3 → v4） (*2026-04-04*)
...
### 研报管线 IR 升级（v3 → v4）新增 (*2026-04-04*)
...
- `scripts/ir_extract_content.py` — 新增 (10.5KB, LLM 正文抽取)
- `scripts/ir_query_rewriter.py` — 新增 (11.7KB, LLM 查询改写)
- `scripts/memory_bridge.py` — 修复（指向 memory_agent/）
- `memory_agent/memory_store.py` — 修复 2 个 ChromaDB 解析 bug
- `output/BP_dd_Pipeline_v4.docx` — 管线文档（43KB Word）

### BPDD 管线 Phase 1-3 连接（2026-04-02） (*2026-04-04*)
