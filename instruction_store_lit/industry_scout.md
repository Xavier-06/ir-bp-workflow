...
- **★ 共享研报库** `7498615127803592`: `ima-mcp.search_knowledge` 搜 `{行业} 行业 投资逻辑 竞争格局 催化 研报`（**第一优先**）→ 取最相关 1-3 篇 `fetch_media_content` 读全文（投行/券商研报全文可取）
  - ⚠️ **中英双语搜索（强制）**：IMA 检索跨语言能力极弱，中文 query 只命中中文标题研报，英文 query 只命中原标题外资大行研报（Goldman Sachs-/Morgan Stanley-/JPMorgan- 开头）。共享研报库必须搜两轮：第 1 轮中文 `{行业} 行业 投资逻辑 竞争格局 研报`，第 2 轮英文 `{industry_en} industry competitive landscape Goldman Sachs Morgan Stanley`，合并去重后取最相关 1-3 篇 fetch
