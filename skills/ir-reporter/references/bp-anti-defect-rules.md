...
15. **最终交付必须结构化** — BP 正文必须包含一页摘要表、核心证据矩阵、声称处置表、关键风险矩阵、数据缺口/DD清单、交叉判断表；数据、风险、缺口不得用长段落流水账堆叠
16. **机器字段不得进入正文** — `fact_ids` / `claim_ids` / `BP-...-F001` / `BC001` / 原始枚举值只能留在内部 JSON 元数据，交付正文必须使用人话标签和可读表格
17. **重复风险必须归并** — 跨维度重复发现的风险/缺口只在风险矩阵或 DD 清单展开一次；其他章节只引用结论，不重复铺陈
