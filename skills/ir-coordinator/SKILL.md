...
9f. **IMA 主力源升级为共享研报库（2026-07-27，v4.8，commit f449e17）** — 四条管线（BP/IR/IC/Lit）IMA 主力源全部升级为「共享研报库」`7498615127803592`（GS/MS/JPM/BofA/Citi/UBS/Bernstein 等投行研报，**全文可 fetch，实测 ✅**，按周分文件夹，每周含 `03_投行报告`=大行研报）。**彻底删除长安投研 `7297585010204027` + 公司调研报告 `7302533890465245`**（库主禁止导出，0% 可 fetch，只能拿 200 字摘要——这推翻了 9b 中"introduction 摘要质量极高直接用"的结论，那两个库本质是拿不到正文的摘要库）。所有角色第一优先搜共享研报库，辅以行研智库/机构调研纪要/精选报告 3 个全文可取订阅库。新增时间过滤纪律：优先最近 30 天内投行研报（超 1 个月参考价值显著下降），标题含日期（如 `-260703.pdf`=2026-07-03）据此判断，大行优先。改动文件：`bp_constants.py`(IMA_KB_IDS/IMA_ROLE_KB_MAP) + 4 条管线的 launcher/profile + 各 `_common_tool_guide.md` + BP 12 角色指令 + Lit 2 角色。**注意：`ic_constants.py` 本身无 IMA 库常量（库 ID 全在 launcher 的 `_build_inline_data_source_guide` 和 instruction_store 里），无需改。**
...
9h. **接入 westock-mcp data_news 个股新闻路由（2026-07-27，commit 2f9e346）** — 腾讯新闻换源（9g）后补强个股级新闻能力。`data_news` 需 symbol（股票代码），`type` 参数：0公告 1研报 2新闻 3全部，返回 title/time/url 列表（实测可用，如 `mcp__westock-mcp__data_news(symbol="sh600519", type=3, limit=10)`）。**接入点**：BP/IR/IC 三条管线 tool guide 路由表 + westock 维度表新增个股新闻行；IR profile Step5 上市公司叠加 data_news；IC launcher company_deep + competitive 分支补个股新闻路由。同时修正 IC tool guide 错误描述（原称 westock"没有新闻内容"，实际有 data_news）。**定位**：data_news 是个股级（需代码，symbol 不确定时先 data_search 检索），行业/通用突发新闻仍走 `tencent_news_search`（9g 降级链），二者互补——data_news 补"某只股票的公告/新闻/研报"，tencent_news_search 补"行业/政策/突发事件"。
9i. **清除子代理不可用的 web_search 引用（2026-07-27，commit f3e2db9）** — v2.3 声称清了 80+ 处 web_search 但**漏了 IR 10 个投研_主笔_*.md（32处）+ _common_tool_guide.md（26处）+ IC 9 个 roles/*.md + ic_profile/ir_profile 研究计划子代理 prompt（19处）等，合计 60+ 处**。子代理没有 `web_search`/`web_fetch` 内置工具，调用会静默失败（web_fetch 直接报错中断）。全部替换为 `search_deep(Bash)`（DDG+SearXNG+Google 三路合并，自动抓正文）。**教训**：清 web_search 必须全管线 grep，不能只清 launcher 和 _common_tool_guide.md——角色指令文件（投研_主笔_*.md、roles/*.md）和 profile.py 里的内联 prompt 同样会被子代理执行。
