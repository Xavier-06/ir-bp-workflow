...
def test_append_search_event_dedupes_urls_but_keeps_query_attempts(tmp_path):
    ...
    event = {
        ...
        "role": "bp_customer_revenue_validation",
        "claim_id": "BC005",
        "query": "测试公司 客户 订单",
        "engine": "web_search",
        "result_count": 5,
...
def test_summarize_search_coverage_counts_by_role_and_claim(tmp_path):
    task_dir = tmp_path / "BP-LEDGER"
    append_search_event(task_dir, {"search_task_id": "BST-001", "role": "bp_customer_revenue_validation", "claim_id": "BC005", "query": "q1", "fetched_urls": ["https://a.example/1"]})
    append_search_event(task_dir, {"search_task_id": "BST-001", "role": "bp_customer_revenue_validation", "claim_id": "BC005", "query": "q2", "fetched_urls": ["https://b.example/2"]})
    ...
    assert summary["by_role"]["bp_customer_revenue_validation"]["unique_queries"] == 2
    assert summary["by_role"]["bp_customer_revenue_validation"]["fetched_url_count"] == 2
...
def test_missing_ledger_falls_back_to_section_search_audit(tmp_path):
    ...
    (task_dir / "bp_section_packages.json").write_text(
        json.dumps({
            "packages": [{
                "section_name": "bp_phase2_customer_revenue_validation",
                "package": {
