...
def test_coverage_failure_creates_repair_tasks(tmp_path):
    ...
    (task_dir / "bp_claim_coverage_gate.json").write_text(
        json.dumps({"claims": [{"claim_id": "BC005", "owner_section": "bp_product_commercial", "status": "unverified", "blocking_gaps": ["missing customer evidence"]}]}, ensure_ascii=False),
        encoding="utf-8",
    )
    (task_dir / "bp_search_plan.json").write_text(
        json.dumps({"search_tasks": [{"search_task_id": "BST-001", "claim_id": "BC005", "owner_section": "bp_product_commercial", "queries": ["客户 订单"], "min_fetched_urls": 2}]}, ensure_ascii=False),
        encoding="utf-8",
    )
    ...
    assert result["tasks"][0]["owner_section"] == "bp_product_commercial"
