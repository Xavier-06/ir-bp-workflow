...
from scripts.ir_subagent_launcher_wb import INSTRUCTION_STORE, IR_SUBAGENT_CONNECTOR_IDS
...
def _run_research_plan(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    # v3.2 (2026-08-04): 研究计划指令从内联 f-string 迁入指令库 ir_research_plan.md
    #（对标 BP v13 指令库统一化）。token 替换模式（__ENTITY__ 等）——指令里大量
    # JSON 大括号与 {entity} 模板示例，不能用 str.format。
    # 模板路径走 launcher 的 INSTRUCTION_STORE 常量；测试可 monkeypatch 本模块属性
    instruction_store = INSTRUCTION_STORE
    template = (instruction_store / "ir_research_plan.md").read_text(encoding="utf-8")
    replacements = {
        "__JOB_ID__": job_ctx.job_id,
        "__ENTITY__": entity,
        "__MARKET__": market,
        "__TICKER__": ticker,
        "__TICKER_DISPLAY__": ticker or "无ticker",
        "__ENGLISH_NAME__": english_name,
        "__QUERY__": query,
        "__BENCHMARK_SKELETON_PATH__": str(tasks_dir / f"{job_ctx.job_id}-benchmark_skeleton.json"),
        "__DATA_PACK_PATH__": str(tasks_dir / f"{job_ctx.job_id}-enriched_data_pack.json"),
        "__PLAN_PATH__": str(tasks_dir / f"{job_ctx.job_id}-ir_research_plan.json"),
    }
...
def _run_research_plan_collect(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    """Phase04 collect v3.2: 读取子代理产出的 ir_research_plan.json。

    v3.2 (2026-08-04): 脚本骨架降级已删除 — research plan 由子代理全权生成
    （指令见 instruction_store_ir/ir_research_plan.md）。子代理未产出/校验失败
    → 硬失败（ok=False），由管线层重试 phase04，不再用脚本骨架糊弄下游。
    Thesis 字段兜底（report_type/valuation_paradigm 等）保留给子代理产出的 plan。
    """
    ...
    if not plan_path.exists() or plan_path.stat().st_size <= 200:
        print(f"  ❌ [ir phase04_collect] 子代理未产出 research plan（{plan_path.name}），"
              f"请重跑 phase04_research_plan 派发子代理", flush=True)
        return {"ok": False, "mode": "ir_research_plan", "phase": "phase04_research_plan_collect",
                "job_id": job_ctx.job_id,
                "result": {"error": "plan_missing",
                           "hint": "重新派发 ir-research-planner 子代理（见 instruction_store_ir/ir_research_plan.md）"}}

    try:
        plan = json.loads(plan_path.read_text(encoding="utf-8"))
    except Exception as exc:
        print(f"  ❌ [ir phase04_collect] 读取子代理 plan 失败: {exc}", flush=True)
        return {"ok": False, "mode": "ir_research_plan", "phase": "phase04_research_plan_collect",
                "job_id": job_ctx.job_id,
                "result": {"error": "plan_unreadable", "detail": str(exc)[:300]}}

    validation = validate_research_plan_ready(plan)
    if not validation["ready"]:
        print(f"  ❌ [ir phase04_collect] plan 校验失败: {validation['errors']}", flush=True)
        return {"ok": False, "mode": "ir_research_plan", "phase": "phase04_research_plan_collect",
                "job_id": job_ctx.job_id,
                "result": {"error": "plan_validation_failed", "errors": validation["errors"]}}

    # Thesis 字段兜底（缺失降级，不阻断）；财务推断保证亏损股不被套 PE 框架
    ...
    plan.setdefault("thesis_source", "subagent_generated")
