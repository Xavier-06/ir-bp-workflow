...
def _run_research_plan(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    """Phase 04: BP 尽调研究计划 — needs_dispatch 模式。

    v5.1: phase03 已完成 presearch → 脚本生成骨架 → 主 AI 读 presearch 数据 enrichment → 合并。
    首次执行：生成骨架 plan → 返回 needs_dispatch + instruction（含 presearch 路径）
    主 AI 读 presearch 数据 + instruction → 输出 bp_research_plan_enrichment.json
...
def _research_plan_enrichment_instruction(entity: str, stage_tier: str, task_dir: Path) -> str:
    ...
    return f"""\
PHASE04 RESEARCH PLAN ENRICHMENT — 主 AI 执行

## 背景
phase03 已完成预搜索，搜索结果保存在 `{task_dir / 'bp_presearch_results.json'}`。
脚本已生成确定性骨架计划（{entity}，{stage_tier} 阶段）。
你需要阅读 presearch 数据、BP 原始内容和骨架计划，输出定制化的 enrichment delta。

## 输入文件
1. **Presearch 数据** (`{task_dir / 'bp_presearch_results.json'}`): phase03 产出的预搜索结果
   - 重点关注 headline_findings（最突出的3-5条信息）、data_gaps（搜索覆盖不足的维度）
2. 骨架计划: `{task_dir / 'bp_research_plan_skeleton.json'}`
3. BP 原文: `{task_dir / 'bp_ocr_text.txt'}`
4. 公司 Profile: `{task_dir / 'bp_step0_profile.json'}`
5. Enrichment 指令库: `instruction_store_bp/bp_research_plan_enrichment.md`

## Presearch 数据消费指南
阅读 presearch 数据时：
- 已覆盖很好的维度 → 降低对应 claim 的 priority（不需要重复搜索）
- 搜索覆盖不足的维度 → 提升 priority + 增强 claim 措辞尖锐度
- presearch 中发现的意外数据点 → 生成 additional_claims 和 strategic_questions
- 数据源质量分布 → 判断哪些维度需要子代理深入搜索

## 执行步骤

1. 读取 instruction_store_bp/bp_research_plan_enrichment.md 了解完整要求
2. 读取 presearch 数据（优先读 headline_findings 和 data_gaps）
3. 读取骨架计划，理解 section/fact/claim 结构
4. 读取 BP 原文（重点读业务描述、市场数据、财务预测部分）
5. 读取公司 profile
6. 基于 presearch 数据，按 instruction 要求生成 4 个增量输出:
   - strategic_questions (5 条，基于 presearch 发现优化)
   - claim_priority_deltas (根据 presearch 覆盖情况调整)
   - additional_claims (presearch 中发现的新线索)
   - excluded_fact_keys (不相关的 fact 裁剪)
7. 将输出写入 `{task_dir / 'bp_research_plan_enrichment.json'}`
8. 用 start_phase='phase04_research_plan_collect' 恢复管线
...
def _run_research_plan_collect(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    """Phase 04 collect: 读取主 AI 的 enrichment，合并到骨架计划。"""
    ...
    if enrichment_path.exists():
        try:
            ...
            print(f"  ⚠️ [phase04_collect] enrichment JSON 解析失败: {exc}，使用空 enrichment", flush=True)
    else:
...
def _run_bp_search_plan_compile(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    if not research_plan_path.exists():
        ...
        if not backfill_result.get("ok"):
            return {"ok": False, "mode": "bp_search_plan_compile", "phase": "phase06_search_plan_compile", "job_id": job_ctx.job_id, "error": f"Auto-backfill phase10 failed: {backfill_result.get('error', 'unknown')}"}
        if not research_plan_path.exists():
            return {"ok": False, "mode": "bp_search_plan_compile", "phase": "phase06_search_plan_compile", "job_id": job_ctx.job_id, "error": "Auto-backfill ran but bp_research_plan.json still missing"}
    try:
        research_plan = json.loads(research_plan_path.read_text(encoding="utf-8"))
    except Exception as exc:
        return {"ok": False, "mode": "bp_search_plan_compile", "phase": "phase06_search_plan_compile", "job_id": job_ctx.job_id, "error": f"bp_research_plan.json invalid: {exc}"}
    ...
    return {
        ...
        "phase": "phase06_search_plan_compile",
        "job_id": job_ctx.job_id,
        "result": {
...
def _bp_section_files(*dirs: Path) -> list[Path]:
    allowed_names = {
        f"bp_phase2_{slug}.md"
        for slug in set(BP_ALL_ROLE_SLUGS.values()) | set(BP_LEGACY_ROLE_SLUGS.values())
    }
    seen: set[str] = set()
    files: list[Path] = []
    for directory in dirs:
        for path in sorted(Path(directory).glob("bp_phase2_*.md")):
    ...
    if review.get("verdict") not in ("PASS", "WARN"):
        ...
        for slug in BP_ALL_ROLE_SLUGS.values():
            for d in (outputs_dir, task_dir):
                if (d / f"bp_phase2_{slug}.md").exists():
                    dimension_count += 1
                    break
...
def _run_bp_investment_judgment(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    for d in (outputs_dir,):
        for dim_file in d.glob("bp_phase2_*.md"):
...
def _bp_role_output_path(outputs_dir: Path, role: str) -> Path:
    return outputs_dir / f"bp_phase2_{BP_ALL_ROLE_SLUGS[role]}.md"
...
def _dispatch_role_specs(task_dir: Path, profile: dict) -> list[dict[str, Any]]:
    ...
    result = [
        ...
        },
        {
            ...
            "output_file": str(task_dir / "bp_phase2_market_supply_chain.md"),
            "key_inputs": {
                "company_name": profile.get("company_name", ""),
                "products": products,
        ...
        },
        {
            ...
            "output_file": str(task_dir / "bp_phase2_competition_positioning.md"),
            "key_inputs": {
                "company_name": profile.get("company_name", ""),
                "products": products,
                "competitors": competitors,
        ...
        },
        {
            ...
            "output_file": str(task_dir / "bp_phase2_dealbreaker_risk.md"),
            "key_inputs": {
                "company_name": profile.get("company_name", ""),
                "products": products,
                "competitors": competitors,
    ...
    for attempt in range(max_retries + 1):
        ...
        if attempt < max_retries:
            ...
            if outputs_dir and outputs_dir.exists():
                for md_file in outputs_dir.glob("bp_phase2_*.md"):
                    try:
...
def _dispatch_completion_instruction(roles: list[str], role_slugs: dict[str, str], next_phase: str) -> str:
    ...
    return (
        ...
        f"- bp_phase2_{{slug}}.md （>100 bytes）\n"
        f"- bp_phase2_{{slug}}-facts.json （>10 bytes）\n"
        f"- bp_phase2_{{slug}}-section.json （>10 bytes）\n"
        f"其中 slug ∈ {{{slug_list}}}。\n"
...
def _run_bp_dispatch_prepare(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    return {
        ...
        "instruction": _dispatch_completion_instruction_sequential(
            sub["role_name"], BP_WAVE1_ROLE_SLUGS, "phase09_dispatch_collect",
...
def _run_bp_dispatch_collect(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    return _collect_with_retry(
        "dispatch_collect",
        lambda: _collect_wave_roles(runtime_root, job_ctx, BP_WAVE1_ROLE_SLUGS, _CORE_ROLES, "phase09_dispatch_collect"),
        job_id=job_ctx.job_id,
...
def _role_outputs_complete(role: str, slug: str, outputs_dir: Path, task_dir: Path) -> tuple[bool, Path]:
    ...
    子代理交付 3 个文件：
      1. bp_phase2_{slug}.md           — Markdown 正文
      2. bp_phase2_{slug}-facts.json   — 事实 sidecar
      3. bp_phase2_{slug}-section.json — 结构化 Section Package sidecar
    ...
    if not output_path.exists():
        output_path = task_dir / f"bp_phase2_{slug}.md"
...
def _collect_wave_roles(runtime_root: Path, job_ctx: JobContext, role_slugs: dict[str, str], roles: list[str], phase: str) -> dict[str, Any]:
    ...
    for role in missing:
        ...
        spawn_receipt_path = task_dir / f"bp_phase2_spawn_{slug}.json"
...
def _run_bp_synthesis_prepare(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    brief_path = task_dir / "bp_phase3_brief_synthesis.md"
    brief_lines = [
...
def _run_bp_delivery_inner(runtime_root: Path, job_ctx: JobContext) -> dict[str, Any]:
    ...
    if use_synthesis:
        ...
        file_map = {
            slug: task_dir / f"bp_phase2_{slug}.md"
            for slug in BP_ALL_ROLE_SLUGS.values()
        }
        file_map |= {
            slug: task_dir / f"bp_phase2_{legacy_slug}.md"
...
class BPProfile(PipelineProfile):
    def __init__(self, runtime_root: Path):
        super().__init__(
            ...
            "phase04_research_plan": lambda job_ctx: _run_research_plan(runtime_root, job_ctx),                 # 04 → needs_dispatch (enrichment, 主AI读presearch)
    ...
    def phase_outputs(self) -> dict[str, list[str]]:
        ...
        return {
            ...
            "phase04_research_plan": ["bp_research_plan_skeleton.json"],
