...
def build_step_brief(task_id: str, step: str, entity: str = '', query: str = '') -> str:
    ...
    brief_lines = [
        ...
        f'',
        f'**3. 企业工商/司法/专利/资质（如有 QCC MCP 可用）**',
        f'→ QCC MCP 工具直接调用（不需要 Bash）：',
        f'- `mcp__qcc-company__get_company_info` — 企业基本信息（注册、股东、高管）',
        f'- `mcp__qcc-risk__get_judicial_documents` — 司法文书',
        f'- `mcp__qcc-risk__get_dishonest_info` — 失信信息',
        f'- `mcp__qcc-ipr__get_patent_info` — 专利信息',
        f'- `mcp__qcc-operation__get_qualifications` — 企业资质',
        f'- QCC 查中国大陆注册企业；境外企业用 web_search 兜底',
        f'',
        f'**4. 通用网络搜索（新闻、行业报告、通用信息）**',
        f'→ `web_search`（WorkBuddy 内置工具，直接用）',
        f'- 不适合结构化金融数据（用 search_gateway）和企业数据（用 QCC）',
        f'- 作为所有搜索的兜底手段',
        f'',
        f'**5. 网页正文深度阅读**',
        ...
        f'| 企业工商/司法/专利 | QCC MCP | web_search |',
        ...
        f'4. **QCC 必查项**（如标的涉及中国大陆企业）：工商信息、司法诉讼、专利、资质',
        ...
        f'- 禁止只用 web_search 做所有搜索——它没有 NeoData 金融数据和 QCC 结构化数据',
