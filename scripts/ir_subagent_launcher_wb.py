...
def build_step_brief(task_id: str, step: str, entity: str = '', query: str = '') -> str:
    ...
    brief_lines = [
        ...
        f'- search_deep（Bash: `from scripts.search_gateway import search_deep`）：搜索 + 自动抓正文一步到位',
        f'- search_many（Bash: `from scripts.search_gateway import search_many`）：多关键词批量搜索',
        f'',
...
def launch_next_wave(task_id: str, entity: str = '', query: str = '', market: str = 'us',
    ...
    for r in dispatched:
        ...
        prompt_body += (
            ...
            f'- ⚠️ fetch权限实测(2026-07-21)：精选报告/行研智库=100%可fetch全文→search后取media_id调fetch_media_content | 机构调研纪要=仅NOTE可fetch | 长安投研/公司调研报告=0%可fetch(库主禁止导出),但intro摘要200-500字质量极高,直接用\n'
            f'- ⚠️ 长安投研搜TXT：加filters=[{{"filter_type":"MEDIA_TYPE_FILTER_TYPE","media_type_filter":{{"media_type":["TXT"]}}}}]，TXT的intro就是机构短评正文(200-300字含数据+标的推荐)，比_导读.docx的音频开头质量高一个量级\n\n'
            f'⚠️ 禁止行为：\n'
