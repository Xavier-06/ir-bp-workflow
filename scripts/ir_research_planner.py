...
def _build_core_questions(entity: str) -> list[dict[str, Any]]:
    return [
        ...
        _core_question(
            ...
            ["step1_industry", "step3_finance", "step5_macro"],
            "决定目标价、风险收益比和核心分歧。",
        ),
        _core_question(
            "Q7",
            ...
            ["step5_macro", "step4_mgmt", "step6_valuation"],
            "决定报告是否充分处理反证和下行情景。",
        ),
    ]
...
def _build_fact_requirements() -> list[dict[str, Any]]:
    ...
    return [
        _fact_requirement("revenue_trend", "收入规模、增速、分部/地区结构", official, ["step1_data", "step3_finance", "step6_valuation"]),
        _fact_requirement("profitability", "毛利率、经营利润率、净利率及变化原因", official, ["step1_data", "step3_finance", "step6_valuation"]),
        _fact_requirement("cash_flow", "经营现金流、自由现金流和资本开支", official, ["step3_finance", "step6_valuation"]),
        _fact_requirement("segment_performance", "核心业务/产品/地区分部表现", official, ["step1_data", "step2_biz", "step3_finance"]),
        _fact_requirement("market_size", "行业规模、TAM/SAM/SOM 或可比口径", industry, ["step1_industry", "step6_valuation"]),
        _fact_requirement("growth_rate", "行业增速、渗透率或需求驱动", industry, ["step1_industry", "step7_insight"]),
        _fact_requirement("market_share", "市场份额、排名或竞争地位", industry, ["step1_industry", "step2_biz"]),
        _fact_requirement("competitive_landscape", "主要竞争者、差异化和替代风险", industry, ["step1_industry", "step8_risk"]),
        _fact_requirement("business_model", "收入模式、客户结构和商业闭环", official, ["step2_biz", "step7_insight"]),
        _fact_requirement("product_matrix", "产品/服务矩阵和关键 KPI", official, ["step2_biz"]),
        _fact_requirement("customer_base", "客户结构、渠道结构、集中度或地域结构", official, ["step2_biz", "step8_risk"]),
        _fact_requirement("pricing_power", "定价权、价格带、折扣率或 ASP 变化", official + industry, ["step2_biz", "step7_insight"]),
        _fact_requirement("unit_economics", "单店/单用户/单产品经济模型或单位利润", official, ["step2_biz", "step6_valuation"], "medium"),
        _fact_requirement("moat_evidence", "品牌、渠道、技术、规模或网络效应护城河证据", official + industry, ["step2_biz", "step7_insight"]),
        _fact_requirement("management_roster", "管理层名单、职位和任期", official, ["step4_mgmt"]),
        _fact_requirement("executive_changes", "近期高管、董事或控制权变化", official, ["step4_mgmt", "step8_risk"], "medium"),
        _fact_requirement("ownership", "股权结构、实际控制人和主要股东变化", official, ["step4_mgmt"]),
        _fact_requirement("incentives", "管理层薪酬、股权激励和绩效条件", official, ["step4_mgmt"], "medium"),
        _fact_requirement("governance_risks", "治理、关联交易、审计意见或合规风险", official, ["step4_mgmt", "step8_risk"]),
        _fact_requirement("balance_sheet", "资产负债表质量、杠杆、现金和营运资本", official, ["step3_finance", "step8_risk"]),
        _fact_requirement("guidance", "公司指引、管理层展望或业绩预测口径", official, ["step3_finance", "step6_valuation"], "medium"),
        _fact_requirement("valuation_multiples", "PE/PB/PS/EV 等估值倍数", market, ["step6_valuation"]),
        _fact_requirement("peer_set", "可比公司名单、口径和状态", market, ["step1_industry", "step6_valuation"]),
        _fact_requirement("dcf_inputs", "DCF/WACC/增长率/利润率核心假设", official + market, ["step6_valuation"]),
        _fact_requirement("target_price_range", "目标价区间、估值区间或隐含回报", market, ["step6_valuation", "step8_master"]),
        _fact_requirement("sensitivity", "估值敏感性、关键假设弹性和下行情景", market, ["step6_valuation", "step8_risk"]),
        _fact_requirement("sotp_inputs", "SOTP 分部估值输入和分部口径", official + market, ["step6_valuation"], "medium"),
        _fact_requirement("bear_case", "空头情景、反向证据和结论推翻条件", official + industry, ["step8_risk", "step8_master"]),
        _fact_requirement("risk_triggers", "会改变投资结论的关键风险触发条件", official + industry, ["step8_risk", "step8_master"]),
        _fact_requirement("regulatory_risks", "监管、政策、诉讼或处罚风险", official + industry, ["step8_risk"]),
        _fact_requirement("competitive_risks", "竞争加剧、替代品、价格战或份额流失风险", industry, ["step8_risk"]),
        _fact_requirement("valuation_downside", "估值下修、倍数压缩和目标价下行空间", market, ["step8_risk", "step8_master"], "medium"),
        _fact_requirement("operating_metrics", "经营 KPI、用户/门店/产能/订单等运营指标", official, ["step1_data", "step2_biz"], "medium"),
        _fact_requirement("industry_kpis", "行业特定 KPI、渗透率、渠道或供需指标", industry, ["step1_industry"], "medium"),
        _fact_requirement("macro_sensitivity", "利率、汇率、通胀、政策等宏观敏感性", ["official_macro_data", "central_bank", "statistics_agency", "reputable_media"], ["step5_macro", "step8_risk"], "medium"),
    ]
...
def _build_section_requirements() -> dict[str, dict[str, Any]]:
    return {
        ...
        "step1_industry": _section_requirement(["Q2"], ["market_size", "growth_rate", "market_share", "competitive_landscape"]),
        "step2_biz": _section_requirement(["Q3"], ["business_model", "product_matrix", "customer_base", "pricing_power", "moat_evidence"]),
        "step3_finance": _section_requirement(["Q1", "Q4"], ["revenue_trend", "profitability", "cash_flow", "balance_sheet", "guidance"]),
        "step4_mgmt": _section_requirement(["Q5"], ["management_roster", "executive_changes", "ownership", "incentives", "governance_risks"]),
        "step5_macro": _section_requirement(["Q7"], ["macro_sensitivity", "risk_triggers"], ["claims", "facts_used", "counter_evidence", "data_gaps", "markdown_draft"]),
        "step6_valuation": _section_requirement(["Q6"], ["valuation_multiples", "peer_set", "dcf_inputs", "target_price_range", "sensitivity"]),
        "step7_insight": _section_requirement(["Q1", "Q2", "Q3"], ["revenue_trend", "growth_rate", "business_model", "moat_evidence"]),
        "step8_risk": _section_requirement(["Q7"], ["bear_case", "risk_triggers", "regulatory_risks", "competitive_risks", "macro_sensitivity"]),
...
def apply_ir_enrichment(skeleton: dict[str, Any], enrichment: dict[str, Any]) -> dict[str, Any]:
    ...
    for sq in plan.get("strategic_questions", []):
        plan["coverage_matrix"][sq["question_id"]] = {
            "owner": sq.get("owner_section", "step7_insight"),
