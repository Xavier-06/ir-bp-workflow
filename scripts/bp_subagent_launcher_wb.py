...
def _build_brief(task_id: str, sub: dict, task_dir: Path | None = None) -> Path:
    ...
    lines += [
        ...
        '- ≥ 3 个实际深读过的 URL（web_fetch 抓到的正文，不是只看 snippet）',
        '- ≥ 3 个独立来源域名（不能全是同一个站点的页面）',
        '',
        '**搜索策略（必须执行）：**',
        '1. **第一轮：广度扫描** — 用 Bash 调 `cd {RUNTIME_ROOT} && python3 -c "from scripts.search_gateway import search; results = search(\'关键词\', prefer=\'multi\')"` 或 web_search 多关键词搜索',
        '2. **第二轮：深度验证** — 对第一轮发现的关键 claim，用 web_fetch 读全文验证',
        ...
        '**禁止行为：**',
        '- 禁止只搜一轮就结束——泛搜一轮不够',
        '- 禁止只搜不读——搜到 URL 后必须 web_fetch 读正文提取事实',
        '- 禁止只用 web_search 做所有搜索',
        '',
        '## 执行要求',
        ...
        json.dumps({
            ...
            'search_audit': {
                ...
                'fetched_urls': ['实际用 web_fetch 或正文深读过的 URL'],
